module.exports = {
    ...
    private: function (req, res) {
        ...
        UserM.findOne(req.session.users[socketId].id).exec(function (err, sender) {
    ...
    public: function (req, res) {
        // Get the ID of the currently connected socket
        var socketId = sails.sockets.getId(req.socket);
        let user_id = req.session[socketId];
        // Use that ID to look up the user in the session
        // We need to do this because we can have more than one user
        // per session, since we're creating one user per socket
        UserM.findOne(req.session.users[user_id].id).exec(function (err, user) {
            ...
            // their socket subscribed to it in RoomMController.create; everyone who joins later gets
            // subscribed in RoomMController.join.
            console.log(user)
            RoomM.message(req.param('room'), {
                room: {
                    id: req.param('room')
                },
                from: user,
